...
$(function() {
  ...
  $('#search-playlisttype').change(function() {
    ...
    if (playlisttype == 'artistdescription') {
      ...
      $('#primary-fieldcontain').attr('title','Descriptive terms to search for, separated by commas (e.g. 80s, cold, funk).')
    }
    else if (playlisttype == 'songradio') {
      primarylabel.text('Spotify song URI(s)');
      $('#primary-fieldcontain').attr('title','Specific song URI(s) (spotify:track:xxx) separated by commas.')
    }
    else {
      primarylabel.text('Artist name(s)/URI(s)');
      $('#primary-fieldcontain').attr('title',"Name of artist or artists to search for. Separate with commas.<br>A single artist can be a name description (e.g. 'Daft Punk') or a Spotify artist URI (e.g. 'spotify:artist:xxx')");
    }
  });
  ...
  // init tooltips
  $('.fieldcontain').tooltip({show: {effect: 'fadeIn',delay: 4000},position: {my: 'left top',at: 'left bottom+11'}});
  ...
  $('#search').find('#search-byartist,#search-bysong').on('click',function()
  {
    ...
    {
      $('#search-primary').siblings('label').text('Artist name/URI');
      $('#search-header').text('Find Songs');
      $('#primary-fieldcontain').attr('title',"Name of artist to search for.<br>A single artist can be a name description (e.g. 'Daft Punk') or a Spotify artist URI (e.g. 'spotify:artist:xxx')");

    }
    else
    {
      $('#search-primary').siblings('label').text('Artist name(s)/URI(s)');
      $('#search-header').text('Find Artists');
      $('#primary-fieldcontain').attr('title',"Name of artist or artists to search for. Separate with commas.<br>A single artist can be a name description (e.g. 'Daft Punk') or a Spotify artist URI (e.g. 'spotify:artist:xxx')");
    }
    if ($(this).attr('id') == 'search-bysong')
  ...
  $('#search').find('#search-byplaylist').on('click',function()
  {
    ...
    $('#search-hotness').siblings('label').text('Artist hotness');
    $('#search-header').text('Generate a Playlist');
    if (Modernizr.sessionstorage)
      sessionStorage.setItem('startsearchsetting','playlist');
  });
  ...
  $('#search').find('input.reset').on('click',function()
  {
    ...
    if ($('#search-header').text() == 'Find Songs') {
      $('#search-primary').siblings('label').text('Artist name/URI');
      $('#primary-fieldcontain').attr('title',"Name of artist to search for.<br>A single artist can be a name description (e.g. 'Daft Punk') or a Spotify artist URI (e.g. 'spotify:artist:xxx')");
    }
    else {
